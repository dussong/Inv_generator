...
//Logfile name
logfile := "logNbody_output_ba.txt";
...
// Gb := GROUP_DEF;
// Examples:
G := PermutationGroup<6 | (1, 2, 3) (4, 6, 5)>; //4B
Gb := PermutationGroup<6 | (1, 2, 3) (4, 6, 5)>;
...
// R0 := [&+[(Q.1)^g:g in Group(Rpre)],&+[(Q.4)^g:g in Group(Rpre)],&+[(Q.1*Q.1)^g:g in Group(Rpre)],&+[(Q.4*Q.4)^g:g in Group(Rpre)],&+[(Q.4*Q.4*Q.4)^g:g in Group(Rpre)],&+[(Q.1*Q.1*Q.1)^g:g in Group(Rpre)]];
// R0 := [&+[(Q.1)^g:g in Group(Rpre)],
//        &+[(Q.4)^g:g in Group(Rpre)],
//        &+[(Q.1*Q.1)^g:g in Group(Rpre)],
//        &+[(Q.4*Q.4)^g:g in Group(Rpre)],
//        &+[(Q.4*Q.4*Q.4)^g:g in Group(Rpre)],
//        &+[(Q.1*Q.1*Q.1)^g:g in Group(Rpre)]];
R0 := [&+[(Q.1)^g:g in Group(Rpre)],
       &+[(Q.4)^g:g in Group(Rpre)],
       &+[(Q.1*Q.2)^g:g in Group(Rpre)],
       &+[(Q.4*Q.5)^g:g in Group(Rpre)],
       &+[(Q.4*Q.5*Q.6)^g:g in Group(Rpre)],
       &+[(Q.1*Q.2*Q.3)^g:g in Group(Rpre)]];
